...
RSpec.describe Meal, type: :model do
  context 'when meal only has a name' do
    it 'is saved successfully' do
      expect(described_class.new({ name: 'Name' }).save).to be true
    end
  end
  ...
  context 'when name is empty' do
    it 'is not saved successfully' do
      expect(described_class.new({ name: '' }).save).to be false
    end
  end
  ...
  context 'recipe book is not set' do
    ...
    it 'returns nil when recipe_book_name is called' do
      expect(described_class.new({ name: 'Name' }).recipe_book_name).to be_nil
    end
  end
  ...
  context 'when recipe book and page number are both set' do
    ...
    it 'is saved successfully' do
      expect(described_class.new({ name: 'Name', recipe_book: recipe_book, page_number: 55 }).save).to be true
    end
  end
  ...
  context 'when recipe book is set' do
    ...
    it 'recipe_book_name returns the correct name' do
      meal = described_class.new({ name: 'Name', recipe_book: recipe_book })
